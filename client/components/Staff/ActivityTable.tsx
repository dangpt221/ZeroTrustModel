      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          <thead className="bg-slate-50/50 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b border-slate-100">
            <tr>
              <th className="px-10 py-4 md:py-6">Thời gian</th>
              <th className="px-10 py-4 md:py-6">Hành động thực hiện</th>
              <th className="px-10 py-4 md:py-6">Trạm truy cập</th>
              <th className="px-10 py-4 md:py-6 text-right">Mức độ rủi ro</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {logs.length > 0 ? logs.map((log) => (
              <tr key={log.id} className="hover:bg-emerald-50/30 transition-colors group">
                <td className="px-10 py-4 md:py-6">
                ...
                <td className="px-10 py-4 md:py-6 text-right">
