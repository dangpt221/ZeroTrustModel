  ...
  if (!user?.departmentId && !isAdmin) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4 md:p-8 animate-in fade-in zoom-in duration-700">
        ...
        <h2 className="text-3xl font-black text-slate-800 tracking-tight text-center max-w-md italic">Quản lý: Chờ phân công bộ phận</h2>
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-8">
        ...
        <div className="bg-white p-4 md:p-8 rounded-3xl border border-slate-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {departmentStaff.slice(0, 9).map(member => (
              <div key={member.id} className="flex items-center justify-between p-3 border border-slate-50 hover:bg-slate-50 rounded-xl transition-colors">
