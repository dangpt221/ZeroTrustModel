...
export async function startServer() {
  ...
  // Public users list (for non-admin features)
  ...
  app.post('/api/admin/users/:id/lock', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') return res.status(403).json({ message: 'Admin only' });
    const { id } = req.params;
  ...
  app.post('/api/admin/users/:id/mfa', authenticateToken, (req, res) => {
    if (req.user.role !== 'ADMIN') return res.status(403).json({ message: 'Admin only' });
    const { id } = req.params;
